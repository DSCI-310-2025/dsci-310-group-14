...
```
![Pairs plot for all variables of interest for exploration.](../results/figures/pairplot.png){#fig-pairplot width=100%}
...
From the @fig-pairplot pairplot visualization above, we see that new vaccination status and time progression (`date`) has positive moderate association with anxiety search trend. Meanwhile, new hospitalization patient rate is negatively and moderately correlated with anxiety search trend. 
...
![Time Series Plot of the Normalized Google Search Volume for Anxiety on a given date.](../results/figures/anxiety_search_time_series.png){#fig-trend width=100%}
